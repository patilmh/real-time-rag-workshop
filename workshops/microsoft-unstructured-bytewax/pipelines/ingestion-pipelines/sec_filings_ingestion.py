...
# Helper function to retrieve ticker for each SEC filing
def enrich(data, cik_to_tickers):
    # get "cik" for each SEC filing
    # Example = ["MORF", {"id": "000141588924021765", "title": "4 - Morphic Holding, Inc. (0001679363) (Issuer)", "link": "...", "cik": "0001679363", "form_type": "4"}]
    cik = int(data["cik"])
    try:
        # Try getting corresponding ticker for the cik
        ...
        if str(data["form_type"]) in ["5", "3", "4"]:
            ...
            # Example - https://www.sec.gov/Archives/edgar/data/1679363/000141588924021765/0001415889-24-021765-index.htm
            parts = data['link'].split('/')

            # Extract the relevant parts from the URL
            # cik = 000141588924021765, accession_number = 0001415889-24-021765-index.htm
            cik = parts[6]
            accession_number = parts[7]
            formatted_accession_number = parts[-1].replace('-index.htm', '.txt')

            # Construct the new URL to retrieve filing text
            # Convert https://www.sec.gov/Archives/edgar/data/1679363/000141588924021765/0001415889-24-021765-index.htm
            # to https://www.sec.gov/Archives/edgar/data/1679363/000141588924021765/0001415889-24-021765.txt
